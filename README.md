...
This repository is an [Open Plugin](https://open-plugins.com) package. Conformant agent hosts discover the manifest at `.plugin/plugin.json` and load skills from `skills/`.

Cursor canvases import only from `cursor/canvas`, a module the Cursor IDE provides at compile time, so they cannot build or render outside the IDE on their own. This plugin provides a **`publish-canvas`** skill that any Cursor agent (including cloud agents) can call to put a canvas online:
...
Install or load this repository as a plugin in a host that supports the [Open Plugin Specification](https://open-plugins.com/plugin-builders/specification). The host discovers:

- Manifest: `.plugin/plugin.json`
- Skill: `skills/publish-canvas/SKILL.md` (namespaced as `cursor-canvas:publish-canvas`)
...
```bash
bash skills/publish-canvas/publish.sh ~/.cursor/projects/<workspace>/canvases/<name>.canvas.tsx
```
...
| Path | Role |
| --- | --- |
| `.plugin/plugin.json` | Open Plugin manifest (name, version, metadata). |
| `skills/publish-canvas/SKILL.md` | The skill definition agents follow. |
| `skills/publish-canvas/publish.sh` | Build + deploy script. |
| `skills/publish-canvas/template/` | Vite app template with the `cursor/canvas` → shim alias. |
